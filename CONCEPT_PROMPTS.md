...
> **Context (this chat has no other background):** *Ouroboros Manifold* is a real-time strategy game where fights are won by positioning, not unit stats. Units balance in a soft rock-paper-scissors (fast units flank heavy ones, heavy ones crush lines, lines absorb fast units) and maps are stacked in elevation levels. This animation is for the section aimed at experienced RTS players. Its goal is to show strategic depth: two skilled players trading moves and counter-moves like a chess game, each with several options at every moment — using only abstract shapes and arrows. It must never look like a battle or show damage; it should read as thinking.
>
> [style block]
